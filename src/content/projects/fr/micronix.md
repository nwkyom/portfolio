---
title: Micronix
company: Micropol
description: "ERP de gestion commerciale leader en Polynésie française. Développement de modules web (Dashboard, PWA inventaire), outils de déploiement Python, maintenance et évolution du legacy."
stack: [Java 21, Spring Boot 3, Vue.js 3, Quasar, Angular, Python 3, Flask, MariaDB, Docker, GitLab CI/CD, PWA, Scandit SDK]
order: 1
icon: erp
locale: fr
---

## Points forts

- Application PWA de relevés de prix et d'inventaires avec Scandit
- Module Dashboard Angular / Spring Boot (ventes, commandes, stocks)
- Suite d'outils de déploiement Python 3 pour Micronix V6
- Intégration APIs NiuPay (paiements) et Revatua (connaissements)
